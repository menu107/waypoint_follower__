...
void process() {        
	...
	if(is_pose_ && is_course_ && is_lane_) {
		...
		if(is_state_change_) {
			...
			if (next_mission_state_ ==10){//원래는 주차 밑에, static에 해당하는 state로 기입해야 함.
				if(is_obs_detect_){
...
//LiDAR한테 PUB
		...
		if(is_control_) {
			...
			else if (next_mission_state_ ==10 && !is_onLane_){//원래는 주차 밑에, curvature위에 static에 해당하는 state로 기입.
				speed = 2.0;
				lookahead_dist_= 4.5;
			}
			/*
