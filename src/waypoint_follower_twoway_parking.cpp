...
void process() {        
	...
	else if((lane_number_!=0)&& next_mission_state_==1){//원래는 주차 밑에, static에 해당하는 state로 기입해야 함. 이를위해 parking_count를 -2로 init함.
		ex_lane_final_ = lane_final_;
		private_nh_.getParam("/waypoint_follower_node/first_state_index", first_state_index_);
		private_nh_.getParam("/waypoint_follower_node/lane_final", lane_final_);
	}
			...
			if (next_mission_state_ ==1){//원래는 주차 밑에, static에 해당하는 state로 기입해야 함.
				if(is_obs_detect_){
					...
					if(is_onLane_){
						...
						if(lane_number_ ==0)lane_number_ = 1;
						else if(lane_number_ ==1)lane_number_ = 0;
						is_lane_ = false;
						is_obs_detect_ =false;
...
//LiDAR한테 PUB
			...
			if (next_mission_state_ ==1){//원래는 주차 밑에, curvature위에 static에 해당하는 state로 기입.
				speed = 2.0;//2.0
                                lookahead_dist_= 2.5;//3.5
			}/*
