...
void initSetup() {
	...
	parking_count_ = -2;//원래는 -1이지만 주차를 고려하지 않기 위해 parking_count를 -2로 init함.
...
double calcSteeringAngle() {
	...
	if( (parking_count_==-2)&&(waypoints_size_>15)&&(next_mission_state_!=1)){//원래는 주차 밑에, static에 해당하는 state로 기입해야 함. 이를위해 parking_count를 -2로 init함.
		transform2local();
...
void process() {        
	...
	if((lane_number_!=0)&& (parking_count_>-2)){
		...
		if(is_state_change_) {
			...
			//cout<<"is state change true"<<endl;
			if (next_mission_state_ ==1){//원래는 주차 밑에, static에 해당하는 state로 기입해야 함.
			//	if(is_obs_detect_!=ex_obs_detect_){
						cout<<"################################obj_detect CHANGED#######################################"<<endl;
						cout<<"ex_obs_detect_ : "<<ex_obs_detect_<<endl;
						cout<<"is_obs_detect_ : "<<is_obs_detect_<<endl;
					if(is_obs_detect_){
				...
				ex_obs_detect_ = is_obs_detect_;
	

			//	}
		
			}
            /*if(dist < 1.0 && next_mission_state_ == 1) {
				while(1){
					if(!is_parking_test_){
...
//LiDAR한테 PUB
			...
			if(next_mission_state_ ==1 && is_onLane_){
				speed = 2.0;
                                lookahead_dist_= 6.5;
			}
			else if (next_mission_state_ ==1 && !is_onLane_){//원래는 주차 밑에, curvature위에 static에 해당하는 state로 기입.
				speed = 2.0;
                                lookahead_dist_= 4.5;
			}/*
			...
			else if((parking_count_==-2)&&(abs(curvature_) > 0.03||abs(cur_steer) > 15)) { //curvature value need to change
